...
public class BinaryTreeVisualizer : MonoBehaviour
{
    ...
    private int GetNodeHeight<TKey, TValue>(TreeNode<TKey, TValue> node)
    {
        if (node == null)
            return -1;
        ...
        where TKey : IComparable<TKey>
    {
        ...
        switch (spacingType)
        {
            ...
            case SpacingType.InOrder:
                ...
                CreateNode(root);
                break;
            case SpacingType.PreOrder:
                nodePositions.Clear();
                int currentXIndexPre = 0;
                AssignPositionsPreOrder(root, 0, ref currentXIndexPre);
                CreateNode(root);
                break;
            case SpacingType.PostOrder:
                nodePositions.Clear();
                int currentXIndexPost = 0;
    ...
    {        
        ...
        currentXIndexRef++;
        AssignPositionsPreOrder(node.Left, depth + 1, ref currentXIndexRef);
        AssignPositionsPreOrder(node.Right, depth + 1, ref currentXIndexRef);
    }

    private void AssignPositionsPostOrder<TKey, TValue>(TreeNode<TKey, TValue> node, int depth, ref int currentXIndexRef)
    {        
        if (node == null) 
            return;
        AssignPositionsPostOrder(node.Left, depth + 1, ref currentXIndexRef);
        AssignPositionsPostOrder(node.Right, depth + 1, ref currentXIndexRef);
        float x = currentXIndexRef * horizontalSpacing;
        float y = -depth * verticalSpacing;
        nodePositions[node] = new Vector3(x, y, 0f);
        currentXIndexRef++;
